...
const cfg = computed(() => qhoGridFromParams(props.params, state.value.length));
...
const potentialData = computed(() => {
  ...
  for (let i = 0; i < cfg.value.n; i += 1) {
    const xi = qhoXAt(i, cfg.value);
    x[i] = xi;
    v[i] = qhoPotentialAtX(xi, cfg.value);
  }
...
const observables = computed(() => {
  ...
  return qhoObservablesFromState(state.value, props.params);
});
...
const yScale = computed(() => {
  ...
  const vScale = vMax > 1e-10 ? 0.7 * rhoMax / vMax : 0;
...
const xMeanLineX = computed(() => (observables.value ? mapX(observables.value.xMean) : mapX(0)));
</script>

<template>
  <svg :viewBox="`0 0 ${width} ${height}`" class="viz">
    <defs>
      <linearGradient id="qhoBg" x1="0" x2="0" y1="0" y2="1">
        <stop offset="0%" stop-color="#f8fbff" />
        <stop offset="100%" stop-color="#eef3ff" />
      </linearGradient>
    </defs>

    <rect x="0" y="0" :width="width" :height="height" fill="url(#qhoBg)" stroke="#ccdaef" />
    ...
    <text x="10" y="16" class="label">|psi|^2 in harmonic well</text>
    <text x="10" y="31" class="label">Norm {{ (observables?.norm ?? 0).toFixed(4) }}</text>
    <text x="10" y="46" class="label">&lt;x&gt; {{ (observables?.xMean ?? 0).toFixed(3) }}</text>
    <text x="10" y="61" class="label">&lt;p&gt; {{ (observables?.pMean ?? 0).toFixed(3) }}</text>
    <text x="10" y="76" class="label">sigma_x {{ (observables?.spread ?? 0).toFixed(3) }}</text>
  </svg>
</template>
